...
// Stronger typing for sections
type Section = 'home' | 'cv' | 'projects' | 'feed' | 'contact';
...
function App(): JSX.Element {
  ...
  // Initialize Visit Counter & Time (client-side only)
  useEffect(() => {
    const storageKey = 'portfolio_visits_v2_reset';
    try {
      const storedVisits = localStorage.getItem(storageKey);
      let currentCount = storedVisits ? parseInt(storedVisits, 10) : 0;
      currentCount++;
      localStorage.setItem(storageKey, currentCount.toString());
      setVisitCount(currentCount);
    } catch (e) {
      // If localStorage isn't available for some reason, fallback to 0
      console.warn('localStorage unavailable, visit counter disabled', e);
      setVisitCount(0);
    }
  ...
  useEffect(() => {
    const fetchRecentData = async () => {
      try {
        ...
        const feedItems: ActivityItem[] = FEED_CONTENT.map((item: FeedItem) => ({
          type: 'POST',
          id: item.id,
          title: item.title,
        ...
        if (res.ok) {
          ...
          console.warn('GitHub API returned non-ok response', res.status);
        }

        // 3. Merge and sort by date desc, take top 4
  ...
  const handleNavigation = (sectionId: Section) => {
    ...
    setTimeout(() => setActiveSection(sectionId), 150);
        ...
        return (
          <div className="space-y-8 pb-4">
            ...
            <div className="bg-black border-2 border-[#00ff41] p-6 shadow-neon font-mono text-sm md:text-base">
              ...
              <div className="mb-2">
                <span className="text-[#00ff41] font-bold">root@marllon:~$</span>{' '}
                <span className="text-white typing-effect">cat user_profile.txt</span>
              </div>
...
I still don’t know at what point humanity decided it was a good idea to abandon the wonderful GeoCities aesthetic in favor of Corporate Memphis.`}
              ...
              <div className="mt-6 animate-pulse">
                <span className="text-[#00ff41] font-bold">root@marllon:~$</span>{' '}
                <span className="inline-block w-3 h-5 bg-[#00ff41] align-middle ml-1" />
              </div>
            </div>
            ...
            <div className="bg-black border-2 border-[#00ff41] p-6 md:p-8 shadow-neon relative overflow-hidden group">
              ...
              <div className="text-[#00ff41] text-xs font-bold mb-6 flex items-center gap-2 uppercase tracking-[0.2em] opacity-80">
                ...
                <div className="h-[1px] bg-[#00ff41]/30 flex-grow" />
              </div>
  ...
  return (
    <div className="min-h-screen text-[#00ff41] selection:bg-[#00ff41] selection:text-black font-['VT323']">
      ...
      {/* Top Decor Bar */}
      <div className="fixed top-0 left-0 w-full h-1 bg-[#00ff41] z-50 shadow-[0_0_10px_#00ff41]" />
      ...
      <div className="container mx-auto px-4 py-8 md:py-12 max-w-7xl relative z-10">
        ...
        <header className="mb-12 border-2 border-[#00ff41] bg-black p-8 shadow-neon relative overflow-hidden flex flex-col md:flex-row justify-between items-end md:items-center">
          ...
          <div className="absolute top-0 right-0 w-full md:w-1/2 h-full cyber-grid-container z-0 pointer-events-none">
            <div className="absolute top-1/2 right-0 w-3/4 h-3/4 bg-[#00ff41]/20 blur-[100px] rounded-full mix-blend-screen -translate-y-1/2 translate-x-1/4" />
            <div className="absolute inset-0 z-10 bg-gradient-to-r from-black via-black/60 to-transparent" />
            <div className="cyber-grid-3d" />
            <div className="absolute inset-y-0 right-0 w-2/3 h-full bg-gradient-to-l from-[#00ff41]/20 to-transparent z-10 pointer-events-none" />
          </div>
          ...
          <div className="w-full relative z-20">
            ...
            <div className="h-2 w-32 bg-[#00ff41] mt-4 mb-4" />
